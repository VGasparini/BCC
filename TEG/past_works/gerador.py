...
def gera_grafo(vertices):
    grafo = [
        [0 for m in range(len(vertices))] for n in range(len(vertices))
    ]  # compressão de lista para criar a matriz grafo[n][m] com todos os valores 0
    for vertice in range(len(vertices)):  # iteração até n
        arestas = set()
        while len(arestas) <= 2:
            aresta = r.randrange(len(vertices))
            if aresta != vertice:
                arestas.add(aresta)  # gera 3 arestas aleatórias
...
def print_grafo(grafo, grafo_pesado, n):
    ...
    for v in grafo:
        f.writelines(
            "".join(str(v)).replace("[", "").replace("]", "").replace(",", "") + "\n"
        )
